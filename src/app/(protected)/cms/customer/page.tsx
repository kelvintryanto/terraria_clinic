...
const CustomerPage = () => {
  ...
  useEffect(() => {
    ...
    const filtered = customers.filter((customer) => customer.name.toLowerCase().includes(searchLower) || customer.email.toLowerCase().includes(searchLower) || customer.phone.includes(searchLower) || customer.address.toLowerCase().includes(searchLower));
  ...
  return (
    <div className="w-full p-3 sm:p-5">
      <div className="mb-4 sm:mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <div className="relative w-full sm:w-64">
            <Input type="text" placeholder="Cari pelanggan..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
        ...
        <Table>
          ...
          <TableBody>
            {filteredCustomers.map((customer, index) => (
              <TableRow key={customer._id.toString()} className="hover:cursor-pointer" onClick={() => handleCustomerClick(customer._id.toString())}>
      ...
      <div className="grid grid-cols-1 gap-4 md:hidden">
        {filteredCustomers.map((customer, index) => (
          <Card key={customer._id.toString()} className="hover:bg-accent cursor-pointer transition-colors" onClick={() => handleCustomerClick(customer._id.toString())}>
