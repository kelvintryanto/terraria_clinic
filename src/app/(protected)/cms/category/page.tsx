...
export default function CategoryPage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      setCategories(data);
      setFilteredCategory(data.filter((cat: Category) => cat.type === activeTab));
    } catch {
      toast({
        title: "Error",
  ...
  useEffect(() => {
    ...
    const filtered = categories.filter((cat) => cat.type === activeTab && cat.name.toLowerCase().includes(searchLower));
  ...
  const categoryToEdit = categoryIdToUpdate ? categories.find((cat) => cat._id.toString() === categoryIdToUpdate) : null;
    ...
    <>
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>{categoryIdToUpdate ? "Edit Kategori" : "Tambah Kategori"}</DialogTitle>
            <DialogDescription>{categoryIdToUpdate ? "Edit kategori yang sudah ada" : "Tambah kategori baru untuk produk atau layanan"}</DialogDescription>
          </DialogHeader>
          <form onSubmit={categoryIdToUpdate ? handleUpdate : handleSubmit} className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="type">Tipe</Label>
              <Select name="type" defaultValue={categoryToEdit?.type || activeTab}>
                <SelectTrigger>
            ...
            <div className="grid gap-2">
              ...
              <Input id="kategori" name="kategori" defaultValue={categoryToUpdate || ""} className="col-span-3" />
            </div>
            <DialogFooter>
      ...
      <div className="w-full p-3 sm:p-5">
        ...
        <Tabs defaultValue="product" className="w-full" onValueChange={(value) => setActiveTab(value as "product" | "service")}>
          <TabsList className="mb-4 bg-background border">
            <TabsTrigger value="product" className="flex items-center gap-2 text-muted-foreground hover:text-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
              <Package className="h-4 w-4" />
              Produk
            </TabsTrigger>
            <TabsTrigger value="service" className="flex items-center gap-2 text-muted-foreground hover:text-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
