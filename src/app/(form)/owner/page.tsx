    ...
    async function fetchCustomerData() {
      try {
        ...
        if (meData.user && meData.user.id) {
          // Then fetch the complete customer data by ID
          const customerResponse = await fetch(
            `/api/customers/${meData.user.id}`
          );
  ...
  return (
    <div className="relative min-h-screen pt-20 px-4 pb-8 md:px-8 overflow-hidden">
      ...
      <motion.div
        ...
        className="max-w-7xl mx-auto space-y-8 flex justify-center"
      >
        {isLoading ? (
          ...
          <OwnerForm initialData={customerData} />
        )}
      </motion.div>
    </div>
  );
